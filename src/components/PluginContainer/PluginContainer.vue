...
export default Vue.extend({
  ...
  watch: {
    currentPluginId: {
      ...
      handler(val: string | null) {
        ...
        const vm = this;
        const previousPlugin: Vue | null = rootRef.$children
          ? rootRef.$children[0] || null
          : null;
  ...
  methods: {
    activatePlugin(prevPlugin: Vue | null, newPlugin: Vue) {
      ...
      if (
        prevPlugin &&
        prevPlugin.$pluginHooks &&
        prevPlugin.$pluginHooks.willUnmount
      ) {
      ...
      if (
        prevPlugin &&
        prevPlugin.$pluginHooks &&
        prevPlugin.$pluginHooks.didUnmount
      ) {
